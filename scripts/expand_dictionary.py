...
def request_bytes(url: str) -> bytes:
    req = urllib.request.Request(url, headers={"User-Agent": "word-logic-relay-dictionary-builder/2.2"})
...
def main() -> None:
    ...
    # Candidate validity comes from the Wordle lists. Frequency sources are used
    # only to prioritize/classify ordinary words, not as a standalone validity source.
    ...
    for word in sorted(candidates):
        ...
        if not pos:
            ...
            pos = ""
        level = "e" if (word in ej_frequency or google_rank.get(word, 999999) <= COMMON_RANK_CUTOFF) else "h"
