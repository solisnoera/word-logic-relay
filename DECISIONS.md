...
- Valid-guess coverage and answer eligibility are separate concerns. All 2,634 embedded words remain valid guesses, but transparent regular plurals/third-person forms, regular past/participle forms, and `-ing` forms are removed from answer pools unless the exact lowercase form has an independent semantic EJDict headword. Irregular inflections are not filtered by this rule.
...
- `dist/assets/answer-exclusions.js` is generated from the pinned morphology/dictionary policy and removes guess-only transparent inflections from RANDOM and DAILY answer selection without affecting input validity.
